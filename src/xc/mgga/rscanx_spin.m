function [ex,vx,v2x,v3x] = rscanx_spin(rho,sigma,tau)
% @file    scanx_spin.m
...
% ==============================================================================================
 ...
 % v3x has 2 cols: [D(\epsilon_x(2n_up)*2n_up)/D(2tau_up), D(\epsilon_x(2n_dn)*2n_dn)/D(2tau_dn)]
    [ex_up, Vx1_up, Vx2_up, Vx3_up] = rscanx(2*rho(:, 2), 4*sigma(:, 2), 2*tau(:, 2));
    [ex_dn, Vx1_dn, Vx2_dn, Vx3_dn] = rscanx(2*rho(:, 3), 4*sigma(:, 3), 2*tau(:, 3));
